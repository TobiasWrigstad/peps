...
*shallow* immutability.) A convenience function
``is_immutable(obj)`` is added to the ``sys`` module and returns
true if an object is immutable.
...
extensions, including support for atomic reference counting on
immutable objects.
...
We choose to make support immutability opt-in. **TODO: description
how that is done.**
...
We choose to make immutability strict.
...
types will not be freezable. This raises the question how to handle
...
1. Freeze fails completely. In the strict interpretation of deep
   ...
   and ``freeze(o1)`` raises an exception.
...
3. **Rejected alternative**: Freeze fails partially. If we permit
   non-strict deep immutability, we could either stop freezing
...
We choose alternative 1, where freezing either succeeds or fails
completely.
...
extensions unless they want to add support for immutability. To
support immutability a class must **TODO**. Depending on how state
is kept and modified, additional changes are needed:
...
their data through Python dictionary objects, no changes are
needed.

Python objects may define a ``__freeze__`` method which will be
called **after** an object has been made immutable. This hook
could then be used to freeze or otherwise manage any other state
on the side that is introduced through a C-extension.
...
*thread-safe*. There are cases where strict adherence to
...
above, the ``freeze(obj)`` builtin works as follows:
...
1. Introduce an ``is_immutable`` function in the ``sys`` module
   that can check whether or not an object is frozen. The status
   is accessible through ``_Py_ISIMMUTABLE`` in the C API and in
   Python code through the ``isimmutable`` builtin.
...
4. The function ``freezeglobals`` in ``sys`` freezes the globals
