...
hashable, due to its pointer being a stable, unique integer associated
with the object for its entire post-freeze lifetime. However, there are
several complications which should be kept in mind for any future PEPs
which build on this work at add this functionality.
...
3. There is one version of the cache per interpreter
