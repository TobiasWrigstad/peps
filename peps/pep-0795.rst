...
2. Extend immutability to user-defined types and their instances – but
   ...
   class variables). Instances of user-defined types will be freezable,
   meaning they will be mutable at creation but may be made immutable.
3. Permit monkey patching of types before they become immutable. This
   stage permits freezable type objects.
...
immutability permits the object to safely forego isolation and
...
introduce a different technique to handle cycles.
...
function. We also make the types ``list``, ``dict``, and
``set`` immutable and their instances freezable, meaning
lists, dicts and sets are mutable on creation but can be made
immutable later.
...
stage 1, they already exist there as part of the immutable type objects.
...
shaped. (The constraint of only freezing self-contained object
graphs was added due to comments from DPO discussions.)
...
To declare an immutable function or type we use a ``@frozen`` decorator,
...
is safe to share across subinterpreters. This means that the function
...
As a companion to the ``@frozen`` decorator, stage 3 adds a
...
arguments to ``@frozen`` and ``@freezable``.
...
-  Freezing a declaration that captures mutable state will fail unless
   the declaration is decorated with ``@frozen`` or ``@freezable`` that
...
class is explicitly declared ``@frozen`` or ``@freezable``. This is to
...
annotating the class as ``@frozen`` or ``@freezable``, the programmer
