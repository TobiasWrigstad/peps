...
imported module. Unless we cater to this case specifically and
permit freezing to propagate to module objects, many case of
freezing would incapacitate modules captured by the functions.
Before showing our solution, we first illustrate the consequences
of treating module objects like any other object when freezing:

.. code-block:: python
   :caption: **Listing 10:** If freezing a function would propagate to any captured module objects. **Note that this is not what we do!**
...
Our solution to this problem is to make code like the one in
Listing 10 work like the one in Listing 9, i.e., as if the import
statement had been written inside of the function body. This is
handled transparently in the pre-freeze hook of the default module
type object.
...
To actually freeze a module, rather than turn it into an immutable
proxy for a mutable module, the module must "opt-in" by overriding its
default freeze hook. For example:
...
This will permit the module to be frozen like a normal object,
and subsequently shared across different sub-interpreters. To
freeze oneself, a module can either freeze its module dictionary
or freeze the entire module object:

.. code-block:: python

    freeze(sys.modules[__name__]) # Freeze module at top-level
...
**FIXME -- the example below may not be true given how Python works when capturing an undefined name**
...
The ``set_freezable`` function and the ``Yes``, ``No``, and ``Explicit`` constants
----------------------------------------------------------------------------------

Sets the freezable status of an object to either ``Yes``, ``No``,
or ``Explicit`` (default is ``Yes``).
