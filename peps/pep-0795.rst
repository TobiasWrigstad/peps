...
This PEP partitions all objects into two categories: **mutable**
...
deeply immutable objects. If a mutable objects is **freezable**,
...
sharing of immutable objects across sub-interpreters.
...
GIL, and its memory managed safely using atomic reference
...
The first two stages permit more and more objects to be made immutable,
...
This PEP partions objects into the categories **mutable** and
...
and 3). The function ``is_frozen()`` can be used to test if an object
is deeply immutable.
...
it performs cycle dectection, immutable objects cannot be part of cycle
detection. By keeping sub-interpreter's cycle detection isolated from
each other, we also avoid having to stop all sub-interpreters to perform
cycle detection. To this end, immutable objects are "removed" from
work-list of the cycle dector.

When objects are immutable at creation, immutable object graphs
cannot contain cycles. Therefore, immutable objects do not need to
participate in cycle detection without any risk of creating memory
leaks. Stage 2 and 3 permits creation of immutable cycles. There
we introduce a :ref:`different technique to handle cycles <scc>`.

There are a handful of types that form cycles in the CPython
...
Adding immutable types through C: ``register_shallow_immutable(typ)``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
been registered as shallow immutable will fail unless its fields
contain deeply immutable objects only.

Note that checking whether a type's fields are mutable or not is
beyond the possibilities of this function. It simply accepts this
assertion from the programmer. If a type was incorrectly
registered as shallow immutable, it could lead to undefined
behaviour, including crashes and memory corruption, when using the
type across sub-interpreters.
...
Stage 2 adds support for making instances of user-defined types
...
By default types are freezable but must be frozen *explicitly*,
as illustrated by Listing 2. (The same holds true for module objects,
but see Stage 3.)

.. code-block:: python
   :caption: **Listing 2:** By default, types must be frozen explicitly.
...
declaration. Functions and objects are implicitly freezable, but
note that freezing an object will fail unless its type is
freezable (or already made immutable). The standard types
``list``, ``dict``, and ``set`` are now freezable, meaning lists,
...
the ``freeze()`` function. To make an object immutable at
construction time, simply wrap its construction inside a freeze
call.

To make a type immutable immediately at creation time, a decorator
``@frozen`` can be used. When a type is declared ``@frozen``, all
its containing methods are implicitly declared ``@frozen`` as
well. The decorators, the ``freeze()`` function, along with other
utilities described later in this document are in a module called
``immutable``.
...
capturing mutable state in functions and types. Note that while we
did not discuss immutable functions much in stage 1, they were
already there as part of the immutable type objects. Since those
functions in those types are part of core Python, it is easy to
ensure that these are well-behaved with respect to this PEP
(without changing them in observable ways).

With freezable objects and ``freeze()`` and ``@frozen`` come the
...
created objects or any mutable arguments -- but it may not capture
a reference to mutable state as part of the function object.
Really, an immutable function is more of a "sharable function".
Making a function immutable does not prevent optimisations etc.
which rely on mutating the code object. However, such optimisation
will need to be carried out in a way that's thread-safe without
...
.. code-block:: python
   ...
   send_to_other_sub_interpreter(die_roll) # Problem
...
object of its defining sub-interpreter. If we pass the
``die_roll`` function object to another sub-interpreter, we have
successfully recreated the problematic situation in Figure 1 where
C is ``die_roll`` and D is ``random``. (Actually, the situation is
technically worse as each function object has a reference to the
globals dictionary of its defining subinterpreter, so
``die_roll.__globals__["x"].append(4)`` will add to the list in
``x``.) Since types consist mostly of functions, and we need to
...
When a function is frozen, we *decouple* is from its defining
context in two ways:

1. We replace its captured globals with a copy that contains only
   the values that the function captured. (So ``random`` in our
   example above, but not ``x``.) The copy is shallow.
...
stack frame, we handle those in the same way: we make a shallow
...
Listing 3 below creates a ``Die`` type which is immutable at
creation. (An alternative to the ``@frozen`` decorator is to make
the type ``@freezable`` and then call ``freeze(Die)`` right after
the class declaration -- this is equivalent.) As neither the
...
.. code-block:: python
   ...
   # freeze(d) -- would have failed since Die is not frozen explicitly
...
Freezing the class freezes its functions.
...
Unfreezable and explicitly freezable objects
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Sometimes a function or object should not be freezable as
...
The ``@unfreezable`` decorator can be used to ensure that the
function is never accidentally frozen.

.. code-block:: python
   :caption: **Listing 6:** Examples of opting out of freezing.
   ...
   c = Cell(42)
   unfreezable(c) # call to decorator function
   freeze(c) # Fails "object is unfreezable"

The ``@freezable`` and ``@unfreezable`` decorators are implemented
in terms of the function ``set_freezable(obj, status=Yes)`` which
is defined in the ``immutable`` module. The function sets the
freezable status of an object. If the status is ``Yes``, then the
object is freezable. If the status is ``No``, it is not. If the
status is ``Explicit`` then the object is freezable but only
through an explicit call to ``freeze()``. If freezing propagates
to an explicitly freezable object, then freezing will fail.
``Yes``, ``No`` and ``Explicit`` are all defined in the
``immutable`` module.
...
.. code-block:: python
   ...
   def frozen(obj):
       set_freezable(obj, Explicit) # Yes would have sufficed
       freeze(obj)
       return obj
...
- Declarations (types and functions) can be made immutable, either by
  calling ``freeze()`` or by declaring them ``@frozen``. For ``freeze()``
  to work on types, the types must first have been made freezable by
  ``@freezable`` or ``set_freezable()``.
...
- Certain types are frozen such as ``dict``, ``list``, ``function``, etc.


Stage 3 – enhancing freeze propagation
--------------------------------------

In stage 3 we add a pre-freeze hook that can be used to hook in
custom logic as part of freezing. We also introduce a mechanism that
lets frozen functions capture module state without freezing it by
forwarding accesses to the module of the current sub-interpreter.
...
frozen as its only argument. The callable may call
``set_freezable`` on this object, or its contained objects to
affect whether freezing will succeed or not.

On user defined types, the definition can directly define a
``__pre_freeze__`` method that will be called as the pre-freeze
hook.

The listing below shows the pre-freeze hook used to ensure
that freezing an *instance* of Foo will freeze the Foo *type*
which requires explicit freezing.

.. code-block:: python
   :caption: **Listing 7:** Freezing propagates.

   def explicitly_freezable(obj):
       set_freezable(obj, Explicit)
       return obj

   @explicitly_freezable
   class Foo:
       @classmethod
       def __pre_freeze__(cls):
           freeze(cls)

   f = Foo()
   freeze(f) # Will succeed even though Foo requires explicit freezing
...
the mutable module object of its current subinterpreter. The
...
This behaviour is different from a function that captures an
imported module. Making such a function frozen would propagate to
freeze the captured module object which might incapacitate the
module. In stage 2, we required module objects to be frozen
explicitly to prevent this as illustrated in the listing below.

.. code-block:: python
   :caption: **Listing 10:** A frozen function capturing an imported module
...
In Stage 3 we change the behaviour of freezing module objects. The
result is that functions like Listing YYY are turned into
functions like Listing XXX by using a pre-freeze hook that is
installed on all module objects.

Before we freeze a module object *m* with name *n* we first make a
copy *m'* of *m* and store that copy in a new ``mut_modules``
dictionary in the ``sys`` module. We then remove all module state
from *m* and turn *m* into a proxy object that transparently
...
This avoids the need to refactor Python code into using imports
inside the functions.

  **FIXME** Do we have any way of actually freezing a module now?
  One possible idea: no freeze-hook on Explicit.
...
- Any object can have a pre-freeze hook that can be used to run
  code just before an object is frozen.
- When a frozen function captures a module object, the module
  object is not frozen, but rather turned into an immutable proxy
  that delegates interaction with it back to the mutable module
  on the current subinterpreter.
...
.. code-block:: python
   :caption: **Listing 12:** Cache in prime factorised implemented using an interpreter-local field.
...
input. The function is called by each subinterpreter on first access of
...
-  An object which is an instance of a type that is de-facto immutable
   ...
   that are mutable. Notable exception: instances of ``type``
...
-  Objects which are explicitly freezable (this includes all types
   and module objects) must be frozen explicitly -- not indirectly
...
Stage 3 adds the pre-freeze hook that can be used to perform operations
on objects right before they become frozen. We use this hook to permit
module objects to be captured by frozen methods without causing the
module to be frozen. Accesses on a frozen module object will be forwarded
to the mutable module on the current sub-interpreter.

**Propagation rules:**

- Same as in Stage 2, but right before freezing propagates to an object,
  its ``__pre_freeze__`` method will be called if it exists.

**Freezing rules:**

- Module objects change from explicitly freezable to freezable.
...
()Effects in pre-freeze hooks are not undone.)
...
- If the function has default arguments, these will become frozen.
...
-  **FIXME** Its namespace dictionary is immutable or can be frozen.
-  **FIXME** Its meta class either is or can become immutable. (We will expand on this at a later date.)
-  The type either does not contain any mutable class state, or that state can be frozen.
...
``set_freezable(o, No)``. The ``immutable`` module that this PEP
...
Stage 2 will expand this list to collections and various hashable
Python types, including:
...
call. (Modulo changes made though ``__pre_freeze__()`` hooks.)

When an object is immutable, we set the bit in its header that is used
...
finds the strongly connected components in the graph that becomes immutable. A
...
The ``@frozen``, ``@freezable`` and ``@unfreezable`` decorators (stage 3)
-------------------------------------------------------------------------
...
.. code-block:: c
   :caption: **Listing 23:** Example of call to ``Py_CHECKWRITE``.
