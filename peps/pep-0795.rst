...
@TODO Tobias: add text about permitting incoming aliases above
...
// TODO:  Can we tell apart nested definitions from things defined outside, and placed here?
//  @freezable
//  class BazImpl:
//     pass
//  class Foo:
//     Baz = BazImpl
//
//  There are slight difference in the names, and in the reference counts?
//  Is that sufficient to do this reliably?
...
// COMMENT (mjp): Bar is allocated freshly each time that code is run.
...
-  ``@freezable`` + ``@frozen`` = ``@frozen``
-  ``@freezable`` + ``@freezable`` = ``@freezable``
-  ``@freezable`` + ``@unfreezable`` = ``@unfreezable``
...
// TODO(mjp): I am very unsure how to implement this?  Is 
// this adding additional state to the function object such that if it is
// frozen, then we check which variables it is accessing, and those don't have
// to be "self-contained"?
// Do I need this if the variables are no longer accessible by another path?
//   def make_count():
//      a = 0
//      @freezable
//      def inc():
...
//   c = make_count()
//   c()
//   freeze(c) # should this work?
// Clearly calling c() again should fail.  But the a variable is only accessed
// by a single rc at the point of freeze.
