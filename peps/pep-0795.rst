...
module object imported in the outer scope. Note that modules
...
A author can allow freezing of their module by setting the
...
freezing the module state. This PEP introduces the ``Proxy``
freezability specific to modules which supports these cases,
by turning the modules into proxy objects that import the
module on demand very similar to the one behaviour proposed
by `PEP 810`_ for lazy imports.

.. _PEP 810: https://peps.python.org/pep-0810/
...
The module could use `escape hatches`_ for its mutable state or
...
This proxy mode basically turns the ``roll()`` function from
**Listing 10** into the ``roll()`` function from **Listing 9**
that moved the import statement into the function. In this way
it is basically the same as the behaviour from `PEP 810`_ for
lazy imports.
...
- Declarations (types and functions) can be made immutable but require an
  explicit call to ``freeze()`` by default.
- The ``@frozen`` decorator can be used to freeze a function or type directly
  after creation.
- The ``@freezable`` can be used to allow implicit freezing of types and
  functions. While the ``@unfreezable`` decorator can be used to prevent
...
- Modules have a special proxy freezability which will delegate any
