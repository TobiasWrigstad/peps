...
attribute. This attribute can be set to ``YES``(meaning that
the object can be frozen) ``NO`` (meaning that the object
cannot be frozen) and finally ``EXPLICIT``, meaning that the object can only be
frozen if it is explicitly passed to ``freeze()``. These constants are defined
in the ``immutable`` module.
...
If ``__freezable__`` is not defined, the default freezability is ``YES``.
If ``__class_freezable__`` is not defined, the default freezability is ``NO``.

Note that once an object is frozen, the values of its ``__freezable__`` and
``__class_freezable__`` are set for life.
...
The listing below shows the effects of freezability using the ``set_freezable()``
function.
...
The ``is_freezable(deep=True)`` function
----------------------------------------

The ``is_freezable`` function checks the freezability status of an
object. It returns ``True`` or ``False`` if the object is freezable
or not, respectively. The optional argument ``deep`` controls whether
the check is shallow or deep.

Whether an object is shallow unfreezable, or explicitly freezable
can be checked by looking at the ``__freezable__`` or
``__class_freezable__`` attributes.
