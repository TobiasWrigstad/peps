...
Python is to facilitate concurren programming, both using
...
The immutable objects that this PEP proposes live outside regions.
Since they live outside of the regions, they can be accessed by
anyone at any time. They can be accessed across threads, and
importantly, objects in regions are permitted to reference
immutable objects. In the regions model, we believe that immutable
objects are a great way for regions to share common data, such as
types.
...
involves disconnecting the object or objects you want to extract
from the remaining objects in the region.) A region can be merged
into another region and regions can also be nested.

Slide 9–17 in our presentation from the Language Summit (available
here: https://wrigstad.com/summit-presentation-final.pdf) show
...
here: https://uppsala.box.com/s/h8gqp59alucov86l8auespvhgz6phiq3.
It should be pointed out that the ``move`` and ``share`` keywords
...
We propose two escape hatches, one which will be delivered in the
future (see `The Big Picture`_ above) and one which will be
delived now. The latter is to hide mutable state in thread-local
storage. When an object is frozen, we permit thread-local data to
stay mutable.
