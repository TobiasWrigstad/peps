...
4. Memory for immutable objects can be safely managed without leaks
   ...
   objects. This is key to sharing objects
...
// TODO: xFrednet Is there some python documentation for the de-facto immutability?
// It seems like this text is assuming that everyone knows that this is a thing and
// what it means for the type to be immutable. I don't think we should explain this
// but make sure there is a link somewhere.
...
2. Extend immutability to dicts, lists, sets and user-defined types
   ...
   to immutable.
3. Empower freezing to make objects immutable in the presence of
...
// REVIEW: xFrednet: The previous paragraph makde little sense to me, probably
// because it's missing an introduction scentence saying what it is about.
// Maybe:
...
Sub 2) each with their own GIL. Now Sub 1 uses its GIL to ensure
that manipulations of object O are carried out correctly. However,
Sub 2 also manipulates object O at the same time, which is assumed
to be safe since it's holding its GIL. Since these GIL’s are different,
...
immutable at the time of creating the composed object.

// REVIEW: xFrednet: I wanted to clarify that this property of freezing a composite
// object is only checked on creation. Meaning that later(stage 2) freezing all contained
// objects will not automatically turn the collection immutable. Is that clear?
...
// REVIEW: xFrednet: I don't understand the prevous start. Did you mean something
// like this?
...
currently de-facto immutable objects are immutable and can therefore
be safely shared across sub-interpreters while being consitent with
...
// REVIEW: xFrednet: If the decorator is only exposed from the
// immutable module, we should probably clarify this. As is this
// sounds like a change to the language.
...
// TODO: I think the paragraph above should clarify if this ``own`` copy
// is a copy to the object, or if x points to the same object. Currently
// this is not clear
...
``__freezable__`` with a value of ``False``. We provide a decorator
``@unfreezable`` that inserts such a field in a declaration.

// TODO: xFrednet: "We" meaning "Python" or "the immutable module"?
...
references from objects that stay mutable*. Note that by normal
...
freezable. Note that because it is strongly freezable, we
...
in this example. (It’s becoming immutable is what causes
``__add__`` to become immutable in the first place.)

// TODO: xFrednet: I don't understand the last scentence in ()
...
// REVIEW: xFrednet: I've added the `__freezable__ = True` to these examples, do we want to keep this?
...
concurrency and not specific to this PEP. On a

// REVIEW: xFrednet: The previous text was incorrect, another sub-interpreter
// would by default still import a new module with it's own state and not
// grab the immutable module version from another sub-interpreter.
...
// TODO: xFrednet: The ``immutable`` module is being mentioned again and again
// but I can't remember reading an introduciton saying why we need it. I think
// this should be realtivly early. (I haven't made any changes in this regard)
...
-  An object can be made impossible to freeze by setting
   ...
   instals this field in declarations.
...
- All the objects reachable from the object can become immutable (or have
  been permitted to freeze), including classes and functions in classes.
- Unless the object is strongly freezable, all references it are
...
-  Its meta class either is or can become immutable. **TODO** Matt J to check please!
...
**not** permits objects captured by the decorated declaration to be
...
to keep complexity down? @MattJ @MattP @Fred
...
**TODO** Should be fleshed out @MattJ
