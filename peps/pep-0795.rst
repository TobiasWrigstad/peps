...
to immutability. At a high-level, the three stages are:
...
``@unfreezable`` that ensures inserts such a field in a
declaration.
...
which would also have to be made immutable, but is alredy immutable
