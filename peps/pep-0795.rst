...
An object's immutability is tracked by a bit in its header. We
will use one more bit to handle cyclic immutable garbage.
Both bits can be added without increasing the size of the
object header.
...
-  Adds a new design that limits freeze propagation for types
...
by picking and unpickling. (See `Performance of immutability`_ for
...
these accesses do not synchronise. In the case of the immutable
object, this is not a problem: its reference count it managed through
atomic increments and decrements, and since its fields are immutable,
they are not subject to data races in the program which could corrupt
the reference counts of sub-objects unless handled using locks or
exchanges. If C was a normal Python object, the unsynchronised accesses
could silently corrupt or crash the Python VM.
...
can be used to test if an object is deeply immutable.
...
cannot contain cycles. Therefore, immutable objects of built-in types
do not need to participate in cycle detection without any risk of creating
memory leaks. Adding support for turning mutable objects of user-defined
types immutable permits creation of immutable cycles. There
...
contain deeply immutable objects only. (**FIXME** This is handled
automatically by the ``is_frozen`` function using the built in
``tp_traverse``.)
...
did not discuss immutable functions much when we limited ourselves
to built-in types, they were
...
frozen as its only argument. The callable may call perform
operations on this object, or its contained objects to
...
which requires explicit freezing, *i.e.,* it shows a possible
implementation of the ``@freezable`` decorator.

.. code-block:: python
   :caption: **Listing 7:** Freezing propagates.

   def freezable(obj):
       obj.__pre_freeze__ = lambda o : freeze(o)
       return obj
...
1. We replace its captured globals with a copy that contains only
   the values that the function captured. (So ``random`` and ``x``
...
through an explicit call to ``freeze()``. Finally, there is
an option ``Proxy`` that is currently reserved for module
objects. If freezing propagates
to an explicitly freezable object, then freezing will fail.
``Yes``, ``No`` and ``Explicit`` are all defined in the
``immutable`` module. By default, classes are explicitly
freezable, *i.e.,* as if ``set_freezable(C, Explicit)``
was called on every class ``C``. Further, modules are
by default unfreezable, *i.e.,* they behave as if
``set_freezable(M, No)`` were called on every module
``M``.
...
imported module. Note that modules are unfreezable by default.
If we allowed freezing to propagate to module objects by default, freezing
would in many cases incapacitate modules captured by the functions.

A module ``M`` can be made freezable by ``set_freezable(M, Yes)`` or
``set_freezable(M, Explicit)``. This permits modules to be shared
across interpreters. Escape hatches can be used to retain some
mutable state in an otherwise frozen module. A module can also be
made *sharable* by ``set_freezable(M, Proxy)``. This behaviour is
described below.

Before showing a sharable module, let's first look at what would
happen if the random module was frozen and had taken no steps internally
to support that:
...
freezable by an explicit call to ``set_freezable``. If the random
module did not internally protect its mutable state,  the module
would not be usable to create more random numbers. To protect its
mutable state against freezing, the module could:

1. Wrap its mutable state in an escape hatch
2. Perform the installation of the escape hatch in the pre-freeze hook
3. Turn itself sharable

A sharable module is an immutable proxy object that delegates all
accesses to it to the (mutable) module on the current sub-interpreter.
A function *f* that performs ``m.g`` where ``m`` is an immutable proxy
object will semantically behave similar to ``__import__(m.__name__).g``,
*i.e.,* look up ``g`` in the current sub-intrepreter's module ``m``.
If the function *f* is passed to sub-interpreter *s* then calling *f*
will cause ``m`` to be loaded in *s* (if it was not already).

Making a module sharable installs a pre-freeze hook on the module
...
.. code-block:: python
    ...
    set_freezable(current_module, Proxy)
    freeze(current_module) # Freeze module at top-level
...
- Declarations (types and functions) can be made immutable, either by
  ...
  to work indirectly on types, the types must first have been made freezable
  by ``@freezable`` or ``set_freezable()``.
...
- When a frozen function captures a module object, the module
  ...
  on the current sub-interpreter.
...
monkey patching and cyclic structures.
...
mutable objects, types, and functions immutable. A pre-freeze
...
and (avoiding) immutable modules.
...
- By default, freezing a module object turns the module object into
  a proxy that delegates to a mutable module object on the current
  sub-intrepreter.
...
- If the function has default arguments, these will become (and can be) frozen.
