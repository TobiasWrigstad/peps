...
The immutable module provides the ``set_freezable(obj, status=Yes)``
function which will control if and how an object is freezable. Most
...
``No`` and ``Explicit`` will be provided from the immutable module.
...
.. code-block:: python
   ...
   freeze(c) # 💥 Fails since the `Cell` class is explicitly freezable

   freeze(Cell)
   freeze(c) # Succeeds since `Cell` was explicitly frozen
   is_frozen(val) # Val is now frozen as well

   val = {"2": "Two"}
   set_freezable(val, Explicit)
   c = Cell(val)

   freeze(c) # 💥 Fails because c references val which needs explicit freezing

   freeze(val) # Works because it's explicitly freezable
   freeze(c) # Works since the referenced value is already frozen

   val = {"3": "Three"}
   set_freezable(val, No)
   c = Cell(val)

   freeze(c) # 💥 Fails because val is not freezable
   freeze(val) # 💥 Fails because val is not freezable

Types are by default explicitly freezable, this also prevents all
instances of the type from being immutable, until the type has been
frozen. Functions by default also need to be explicitly frozen, this
is done to prevent accidentally freezing values captured by functions.
Some functions will also break, when they're frozen, for example
functions that modify captured state using the `global` or `nonlocal`
keywords. Note that freezing a type will propagate to the methods
without needing to call freeze on each method explicitly.
...
The ``immutable`` module will provide decorators to easily change
