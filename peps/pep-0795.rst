...
We propose that all functionality is exposed via a new built-in
module called ``immutable``. This allows each Python implementation
to decide if they wants to provide immutability or not. We have a
...
Register immutable types through C: ``register_shallow_immutable(typ)``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
fields are deeply immutable or fail otherwise. This check is done
on the first call of ``is_frozen`` on an object, this means that
...
registered as shallow immutable, it can lead to undefined
...
``dir()`` and ``tp_traverse``, are not sufficient since they don't
guarantee to discover all reachable objects. For this we add a new
function ``tp_reachable`` to C-types. This function needs to be
implemented by a type to support immutability. It will have the same
function signature as ``tp_traverse`` which will allow easy migration
