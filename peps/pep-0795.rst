...
objects that is consistent across both sub-interpreters
and free-threaded Python (and sub-interpreters
running free-threaded). 
...
enforce the envariants necessary for safety. There is a
short note about this at the end of this document.

About the structure of this PEP
-------------------------------

This PEP describes three stages of immutability support in Python. The first
two stages permit more objects to be immutable, and the third stage permits
freezing to propagate further. Permitting more objects to be
immutable requires more changes to the Python interpreter, and
additionally comes with design considerations at the language level.
...
2. Extend immutability to dicts, lists, sets and user-defined types
...
3. Permit freezing of an *object* to propagate to freezing its *type*,
   *supertype*, and their *functions*.
...
objects can be shared directly. This is possible because
...
Recall that all mutable objects live inside
...
named tuple, ``frozenset``, ``range`` and ``slice``
...
With freezable objects and ``freeze()`` and ``@frozen`` comes the issue of
*freeze propagation*, meaning what freezable objects can be made
immutable as a side-effect of making something immutable. To limit freeze
...
reference to ``C`` in ``y``. However, if that reference is removed, or
if ``C`` was already immutable, ``freeze(x)`` will succeed. Note the
...
now. Later, we will show how escape hatches can be used
...
To protect a function from being frozen we provide a decorator
``@unfreezable`` that ensures that attempts to freezing the
function fails.
...
Stage 3 – controlling freeze propagation
----------------------------------------

Stage 3 relaxes the constraints on how freezing can
propagate and adds tools for documenting and maintaining
immutability in code.

In stage 2, types and declarations were only permitted to
capture immutable objects. In stage 3, the ``@frozen``
decorator takes an additional white-list argument that
permits freezing to also freeze the state named in the
white-list. A catch-all argument ``"*"`` to the white-list
permits any captured state to be frozen.

A new decorator ``@freezable`` is added as a companion to
``@frozen``. This decorator captures the default behaviour
of classes since stage 2, but permits white-listing.
The white-list arguments given to ``@freezable`` are stored
in the class and are used by a subsequent call
to ``freeze()``.

Finally, the `module` type object is made immutable which
permits entire modules to be frozen. A module object may
declare itself freezable by setting a ``__freezable__``
...
.. code-block:: python
   ...
   class Person:
       def __init__(self, name): # "inherits" @freezable("monty") from Person
           self.name = name
		   monty.append(self)
...
capture the contents of the ``monty`` variable, freezing a person
object freezes the list of persons, along with all persons in the
list.

We will use the following running example to explain what
happens when we freeze types and functions in stage 3. We
start by explaining the simplest and most permissive model
of freezing, and later discuss how to limit how freezing may
propagate through a program.

**TODO** Fix below example. Everything is implicitly @freezable now.
...
- Support for freezing mutable objects captured in declarations
  through white-listing through arguments to ``@frozen`` and ``@freezable``.
- Freeze propagation is relaxed so that freezing an object can freeze a type,
  and freezing a function can freeze all its called functions.
- A module may declare all its members freezable

**TODO** Should module freezable set the default argument to decorators?
Can the relaxed freezing be expressed as implicit arguments to decorators?
...
Stage 3 permits users to allow freezing to propagate
further. The ``@frozen`` and ``@freezable`` decorators
take optional arguments that identify objects that may be
made immutable as a side effect of making the decorated
declaration immutable. We refer to this as "white-listing".
The special argument ``"*"`` is a catch-all white-list that
permits any object encountered during freezing to be frozen.
(Note that may still fail -- this is a license to call
``freeze()``, not a guarantee that the call will succeed.)

**Propagation rules:**

-  Freezing an object propagates to other objects, but not to
   declarations (i.e. types and functions).
-  Freezing a function is permitted to freeze any function it calls. 
-  Freezing a type propagates to its super types and functions in the
   type and supertypes.
-  Freezing a declaration decorated with ``@frozen`` or ``@freezable``
   may propagate to objects if explicitly permitted by the programmer.

**Freezing rules:**

-  Module objects must opt-in to support freezing.
-  Freezing a declaration that captures mutable state will fail unless
   the mutable state is in the declarations's white-list.
...
-  All the objects reachable from the object can become immutable (or have
   been permitted to freeze) -- including types
...
-  The function either does not capture any mutable objects, or it is
   declared ``@frozen`` or ``@freezable``, the state that becomes immutable is
...
-  The type either does not define any mutable class state, or it is
   declared ``@frozen`` or ``@freezable`` and all mutable objects it
...
class state, meaning we happily make it immutable. We also
do not consider implicitly defined mutable state such as the
``__bases__`` field or the ``__class__`` field.
...
state is explicitly permitted to be frozen throught white-listing, or the mutable state is
implemented through an escape hatch. This is to avoid
accidentally incapacitating a class by freezing it. Thus,
the following class *cannot* be made immutable:
...
This can be solved through white-listing:

.. code-block:: python

   _constants = { 'pi' : 3.14 }

   @frozen("_constants") # or @freezable("_constants")
   class Foo:
       constants = _constants
...
**TODO** Is the first sentence true still?
...
to do so by adding a ``@frozen`` or ``@freezable`` decorator. In other
...
(Techncially, it is also possible to call ``frozen`` as a function
although that it not intended.)

**TODO** Can we check that __freezable__ = 1 in module in stage 2??
Below pbly needs updating.
...
For the same reason, we cannot make ``A`` and ``B`` ``@frozen``. Dealing
...
The ``is_frozen`` function inspects the bit in the object header set by
the ``freeze`` function and returns ``True`` if this bit is set. If
called on a shallow immutable object, and the bit is not set,
``is_frozen`` will call ``is_frozen`` on all the members. If the result
...
The ``@frozen`` decorator behaves exactly like ``@freezable``, except
...
For clarity, the first line means that two ``@frozen`` decorators is
equivalent to a single ``@frozen`` decorator. Note that nesting
something ``@unfreezable`` inside something that is ``@frozen`` raises
...
Arguments are always joined together, so if ``@frozen("a", "b")`` is
...
**NOTE** This section and subsections is WIP

With respect to changes to core Python, stage 2 and 3 are
almost identical. Stage 3 contains some additional features in
the immutable library that do not affect core Python, plus
minimal changes to make the module type object immutable and
module objects freezable. The list below show the changes to
core Python:
...
- Making modules and standard types.
