...
graphs. This ensures that the following will never raise
...
- The function either does not capture any mutable objects,
  or it is declared `@frozen` or `@freezable`, names the
  state that gets frozen explicitly, and those objects
  freeze successfully. Default arguments are always permitted
  to freeze.

If a function captures another function that is not frozen,
the second function will be frozen implicitly. Each of
the following functions can be frozen by a simple call to
`freeze`:
...
The final example shows that freeze propagation does not try
to reason about the meaning of freezing a function. Clearly,
freezing `function4` means that the function will always
throw an exception as it tries to append to an immutable list.
(For the record, this kind of reasoning is not possible
in the general case and the most consistent behaviour is
to simply freeze the function.) To ensure that freezing
`function4` cannot be frozen, the `immutable` library
provides a `@unfreezable` decorator.
...
this bit is set. If called on a tuple, named tuple, or a
frozenset, and the bit is not set, `is_frozen` will call
`is_frozen` on all the members. If the result is true, it
will be "cached" in the object by setting the bit.
...
**TODO before posting PEP:** make a decision.
...
We will use two bits per Python object: the first will be
used to track immutability; the second will be used by the
SCC algorithm.
...
the immutability bit, and using atomic operations only
if the bit it set.
