...
Note that tuples, frozensets, and named tuples are only
considered immutable in this PEP if they only contain other
immutable objects. Note also that the types objects for all
these listed types are immutable too, meaning they cannot
be modified by metaprogramming. (This is already the case in
Python today.)
