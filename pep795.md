...
the less common form above, our ability to reliably analyse
...
An shared field is shared across subinterpreters, which means that
...
permit an shared field to store mutable objects, as the problem
...
global instance counter, we can use an shared field to implement
the caching. Since an shared field cannot hold mutable objects, we
...
freezing. We can implement this using an shared field in
...
subclasses are shared, we add an shared field with an immutable
...
problem -- imagine implementing them using an shared field
...
- `shared`
- `disabled`
- `@freezable` --- add Fred's idea of naming variables to be frozen
- `@unfreezable`
- `make_module_shared`
- `import_as_shared`
