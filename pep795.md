...
are immutable in Python 3.14. All instances of
the following type: `int`, `float`, `complex`, `bool`,
`str`, `tuple`, `frozenset`, `bytes`, `NoneType`,
`Ellipsis`, `range`, `slice`, and named tuples. The
type objects `object` and `type` are also immutable.
While tuples, frozensets, and named tuples offer only
shallow immutability, they can be used to create deeply
immutable objects. Note also that the types objects for all
these listed types are immutable, meaning they cannot be
modified by metaprogramming. (This is already the case in
Python today.)
