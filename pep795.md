...
This PEP proposes support for immutable objects in Python.
An attempt to mutate an immutable objects will fail and
raise an exception. Some objects are *inherently immutable*
meaning they are immutable at creation, for example an
`int` or a tuple of two ints. Some objects are *freezable*
meaning that they are mutable at creation, but can be made
immutable later. Finally, some objects are *inherently
mutable* meaning they are mutable at creation, and cannot be
made immutable.

The immutability that this PEP discusses is *deep*.
This means that for an object to be immutable, it may
only refence other immutable objects (modulo some escape
hatches). Thus, `(42, 4711)` is immutable, but `(42, [])`
is not, as it is possible to obtain a reference to the
tuple's second value and e.g. append to it. (If the list was
immutable, the tuple would be immutable too.) 
...
of an immutable object. Unless stated otherwise, we will
use immutability to refer to deep immutability as described
in this PEP, unless otherwise indicated. of an immutable
object.
...
This section describes the immutability semantics. All
user-facing functions in this PEP are in the module
`immutable`. Across all stages, immutability is *deep*
meaning that an immutable object may only refer to other
immutable objects. As this can sometimes be too strong,
this PEP provides two powerful escape hatches that will be
described further down.


## Stage 1 -- inherently immutable objects

In stage 1, we consider only inherently immutable objects,
i.e. all instances of the following type: `int`, `float`,
`complex`, `bool`, `str`, `tuple`, `frozenset`, `bytes`,
`NoneType`, `ellipsis`, `range`, `slice`, and named tuples.
Note that `tuple`, `frozenset`, and named tuples may only
contain other immutable objects. Note also that the types
objects for all these listed types are immutable too,
meaning they cannot be modified by metaprogramming. (This
is already the case in Python today.)

With this design, immutability is always establised at the
point of creation, which means that there is no need for a
`freeze()` function that takes a mutable object and makes
it immutable.
...
n = ({'x' : 'parrot'}, None)
is_frozen(n) # returns False -- dict is mutable
...
the changes proposed by this PEP. As will be explained in
detail below, as the `n` tuple contains a mutable
dictionary it is deeply immutable and therefore not safe
to share between subinterpreters even with the changes this
PEP proposes.


### Sharing of types

The inherently immutable objects' types are immutable too.
This permits these type objects to be shared across
subinterpreters, and an object's `__class__` references
another immutable object. This is consistent with the
definition of deep immutability -- as soon as we follow a
reference from a mutable object inside a subinterpreter's
heap to an immutable object, we leave the subinterpreter's
heap, and there are no references that we may follow that
lead back to a subinterpreter heap.
...
important, as each subinterpreters operates under the
...
## Stage 2 -- supporting user-defined types and freezing mutable trees

Stage 2 adds support for immutable function objects and
immutable objects of user-defined types. That a function
is immutable means that it may not capture any mutable
state. An immutable function may still mutate any mutable
state it can get a reference to. An immutable type also may
not capture mutable state, and also does not permit adding,
removing or changing functions. Immutable types may not
inherit from mutable types, but may have mutable subtypes.
**NOTE: `object` is immutable**

Instances of immutable types become mutable, and can be
made mutable by freezing them through an explicit call to
a `freeze()` function, but this is only supported if the
immutable object has a reference count of 1 (at the point
of the call), and the object only refers to other immutable
objects, or mutable objects with a reference count on 1.
(Instances of mutable types can never become immutable.)
This ensures that a mutable object will not suddenly become
immutable "under foot", and also avoids the creation of
cycles. Making an object immutable means that subsequent
attempts to write to its fields will result in `TypeError:
object of type T is immutable`.)
...
a module object is not possible since the `Module` class
is not `@frozen`.
...
d.change_sides(12) # will raise an exception
...
* Instances of immutable types are mutable at creation but can be made immutable if they have a reference count of 1
* Making an object immutable blocks subsequent mutation of the object


## Stage 3 -- freezing of arbitrary graphs

Stage 3 permits arbitrary object graphs, including those
with cycles, to be made immutable. This lifts the restriction
on `freeze()` to fail to freeze objects which have incoming
aliases. **NOTE: maybe this should be another function?**
In addition to the `@frozen` decorator, stage 3 adds a
`@freezable` annotation which permits the creation of mutable
classes and functions, which may be e.g. "monkey-patched"
before eventually becoming immutable. It also adds an
`@unfreezable` decorator to explicitly opt-out of support
for freezing. A module may declare all its members freezable
by setting a `__freezable__` field to the value `1`.
**NOTE: should we have a freeze function that fails if it
finds incoming references to the stuff that's frozen?**
...
immutability is **deep** (modulo the aforementioned escape
hatches). We will use the following running example to
explain the semantics of freezing. We start by explaining
the simplest and most permissive model of freezing, and
later discuss how to limit how freezing may propagate
through a program.
...
objects" such as modules, functions, and types. We will use
...
be immutable too, or they are no longer callable. As in
stage 2, making a function object immutable means that the
function is not permitted to capture mutable state. Of the
...
* Object graphs may have any shape -- not just trees
...
freeze the `Fraction` class. If `gcd` captured parts of its
defining module, then those parts would be made immutable
too.
...
Stage 2 adds support for making objects immutable which are
instances of inherently immutable classes, which in turn
consist of immutable function objects.
...
If freezing `x` could propagate to `y`, then by definition
the reference count of `y` is at least 2, which will cause
`freeze(x)` to fail. Similarly, since passing a reference
to a function guarantees that its reference count is at
least 2, the function cannot freeze its arguments. This
is a guarantee that "cuts both ways".

While this design will permit creating a (singly) linked
list of mutable objects and then making the entire list
immutable by freezing the first link, it is very hard to
pass an object around without losing its ability to be
frozen.
...
- Objects that are instances of immutable type support
  freezing by default, but freezing will fail unless the
  instance's reference count is 1.
- An object can be made inherently mutable by setting
  `__freezable__ = 0`.
...
Making an object inherently mutable can be done through a
field `__freezable__` that holds any other value than `1`,
or by using an `@unfreezable` decorator on a declaration.
...
- It is declared in a module that opts-in support for
  ...
  or the function is declared `@freezable`
...
If the constant state is not an inherently immutable object,
...
To make an object inherently mutable (which will prevent
freezing), simply set `__freezable__ = 0` (or any other
value than 1). The `immutable` module that this PEP
provides contains a  context manager that one can use to set
`__freezable__ = 0` at the start of a block, and remove it
at the end:
