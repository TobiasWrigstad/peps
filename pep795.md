...
In summary, we would end up with the following freeze logic:
...
def freeze(objs):
    for obj in objs:
        if is_frozen(obj):
            return
        if isinstance(obj, types.ModuleType) and not obj in objs:
            raise "Encountered mutable module"
...
def freeze(objs):
    for obj in objs:
        ...
        if isinstance(obj, types.ModuleType) and not obj in objs:
            raise "Encountered mutable module"
        if isinstance(obj, type) and not obj in objs:
            raise "Encountered mutable type"
