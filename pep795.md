...
unless the class is explicitly declared `@freezable`. This
is to avoid accidentally incapacitating a class by freezing
it. Thus, the following class cannot be frozen:
...
Although `3.14` was already an immutable object,
declaring the class as freezable prevents reassigning the
`constant_state` field.
...
programmer gave explicit permission to do so by adding an
`@freezable` decorator. In other words, freezing such
_declarations_ should not lead to accidental freezing of
_program state_. Freezing a function cannot "escalate" to
freeze types or modules, and freezing a type cannot escalate
beyond its supertypes (without explicit opt-in).
...
call any of its methods that call `Func1`. We could
go in and freeze `Var1` to resolve this, but we might
be discouraged by code comments, documentation, a
"private name" etc. Freezing `Module3.Type4` will freeze
`Module3.Type3`, but cannot end up freezing `Func3`
or `Var3`. Finally, freezing `Func3` explicitly cannot
propagate through the import to `Module3` since freezing a
function cannot escalate to a module (unless `Func3` was
declared `@freezable`).
...
after its definition has finished executing.
...
decorated. The list below explains what happens when
decorators become nested. The leftmost is the outer
decorator and the rightmost the inner.
...
decorators "collapse" to a single decorator. Note
that nesting something `@unfreezable` inside something
...
`@freezable` annotation, freezing a function will fail if it
...
lead to freezing the contents of a variable outside of the
permitted capture list.
...
permission set is written `@freezable("")` or `@freezable()`.
