# Deep Immutability for Sharing and Safety

This PEP proposes that Python objects can be made immutable after
which they cannot be mutated. We will refer to the act of making
an object immutable as "freezing it". There is no way to make an
immutable object mutable again. However, one can make a mutable
copy of an immutable object.

The motivation for immutability is three-fold: 
...
3. Since immutable objects are safe from data races, it is
   possible to optimise how they are handled in the Python
   run-time under free-threading.
...
All user-facing functions in this PEP are in the module `immutable`.
The most important function is `freeze()` that makes an object immutable.
...
to its fields will raise an exception. Furthermore, by default,
immutability is **deep**. This means that an immutable object is
only permitted to point to other immutable objects. This has
consequences in Python as functions, modules and types are objects
too. We will use the following running example to explain the
...
Let us first focus on "normal Python objects", meaning we ignore
modules, functions, and types.

Right before freezing, we have the following normal object graph.
...
Lines drawn in blue denote pointers that are stored in a field of an
...
object. (As happens in the next line of the code.)
...
graph TD;
  ...
  DD:::imm
  classDef imm fill:#ADD8E6,stroke:#00f,stroke-width:1px;
  linkStyle 1,2,6,8,9,12,13,14,15,16,19,20 stroke:#00f,stroke-width:1px,color:blue;
```
...
Making `Fraction` immutable also makes all function objects,
`__init__`, `__add__`, and `__repr__` immutable. Making a function 
object immutable means that the function is not permitted to 
capture mutable state. Of the three functions in `Fraction`, 
only `__init__` captures external state: a reference to `gcd`. 
This means that the `gcd` function must be made immutable too.
Notably, `__add__` has a reference to `Fraction` which would 
also have to be made immutable, but is alredy immutable in
this example.
...
made immutable just like above. If `gcd` used parts of its
defining module, then those parts would be made immutable too.

What if `gcd` had been called from `__init__` like this:
`useful_stuff.gcd()`? A naive implementation would have first
frozen the entire module object, which would have propagated to
`gcd`. A more refined way is possible: when we freeze `__init__`,
we extract `gcd` from useful stuff, and ignore the module object
altogether.


## Design considerations due to subinterpreters

Python subinterpreters currently do not permit sharing objects by
reference. Every subinterpreter uses its own GIL to ensure that
reference manipulation and cycle detection cannot be corrupted due
to concurrency in the Python program.
...
essence, the fact that an object is immutable permits the objects
...
"worlds" are kept apart.

This explains (at a high-level) why an immutable function may only
call other immutable functions. Recall that all mutable objects live
inside their creating subinterpreter. Assume it is possible to share an
immutable object (including function) with a reference to a mutable
object O in subinterpreter A with subinterpreter B. Now both A and B
can access the shared mutable object -- A uses its GIL to ensure that
manipulations of O are carried out correctly, while B uses its GIL. 
Since these GIL's are different, operations that must be atomic
may in fact not be.
...
mutable and reside in the heap of Subinterpreter 1. Object B is
mutable and resides in the heap of Subinterpreter 2. A and B both
point to an immutable shared object. The reference from C to D is
not legal, and this example shows what happens if it was allowed.
The colours on the dashed arrows show which subinterpreter is
using a reference to access an object. We annotate the accesses
with what GIL is used to synchronise actions on the object.
Because of the reference from C to D, it is possible to have two
accesses with different GILs, meaning these accesses to not
synchronise.
...
have seen, escaping from the immutable world back into the mutable
world is not easy. Even though the fraction instances are themselves
immutable, the fact that they can reach mutable state is problematic
since we want to avoid the problem above.

We have two approaches to solving problems like this which we also
make available through the `immutability` library that is part of
...
assignment. The field takes care of synchronisation if multiple
subinterpreters read or write the field at the same time.
...
class Counter:
    def __init__(self, a, b):
        self.imm = a # will be immutable when frozen
        self.mut = immutable.atomic_field(b)
    def increment(self):
        while True:
            old = self.get()
            new = old + 1
            freeze(new) # atomic fields can only hold immutable objects
            if self.mut(old, new): # stores new in mut if mut's value is old
                return # break out of loop on success
    def reset(self):
        return self.mut(self.imm) # sets the value of the field, returns old value
    def get(self):
        return self.mut() # gets the value of the field
    def __repr__(self):
        return f"({self.imm}, {self.mut()})"
...
Note that we could have used an atomic field to implement the
caching. Since an atomic field cannot hold mutable objects, we
...
As one more example of using atomic fields and interpreter-local
fields, we can revisit the list of references from a superclass
object to its subclasses. If this list was implemented in Python,
we could use a combination of an atomic field and an
interpreter-local field. Consider the immutable class A, and
adding an immutable subclass B and a mutable subclass C. Since
immutable subclasses are shared, we can add an atomic field with a
frozen list of references to frozen subclasses. Every time we add
to that list, we create a new list from the old, add the new
...
visible to the subinterpreter that defined it. Thus, we can use
interpreter-local field to keep a (mutable) list of the mutable
subclasses of A. Together, these two lists make up the subclasses
of a class.
...
that subinterpreter will call the function with `sharable_field`
as its argument, which in this example causes
...
This requires that all objects in the state are immutable.
...
This module is now safe to freeze -- the `messages` variable will
effectively remain mutable (through an indirection), and can be
updated with new messages. Importantly, there will now only be a
single logger shared across all subinterpreters. (To that end, we
should add a lock or a compare-and-set loop to `log` but that is
standard concurrency and not specific to this PEP. Locks are
considered immutable so pose no problem.)


## Keep one logger per subinterpreter

If we have a subinterpreters-enabled module, then it may make more
...
The following classes and functions should be considered immutable
in Python if this PEP is accepted:
...
What is the right semantics is impossible to say until there is
more experience with immutability in Python. Thus, we should pick
a reasonable semantics now, and possibly revise at a later date.
...
The most permissive definition of freezing will walk the object
graph from the argument to the `freeze` call and make every
reachable object immutable. (This was the semantics described at
the top of this PEP, with our home-rolled `Fraction` class.)
In terms of our example graph above, `freeze(y)` under this
model would freeze `O2` and all the contents of `Module1`
including the nested `Module2`. Since `Func1` writes `Var1`, 
this function should no longer be called. The same holds for
all the functions in `Type2` that write `Var2`.
...
## Do not implicitly freeze module objects

**TODO** Revisit this and rewrite it without the open questions.

Freezing a module can have big consequences. In particular, if the
module is not written with immutability in mind. To limit
freeze-propagation, one possibility is to stop (and fail) if
freezing encounters a non-frozen module object. To freeze a
module, the module must be explicitly frozen.

In terms of our example graph above, `freeze(y)` under this model
would attempt to freeze `O2`, then `Func1` which would fail
because the module object that holds `Var1` is mutable.
**THIS SOUNDS WRONG**

For `freeze(y)` to succeed, the programmer would first have to
call `freeze(m1)` which would freeze all the contents of `Module1`
including the nested `Module2`. Again, since `Func1` writes
`Var1`, this function should no longer be called. The same holds
for all the functions in `Type2` that write `Var2`. (Of course, it
is possible that these functions use `immutable.is_frozen()`
internally to avoid the mutation after freezing.)

Requiring modules to be explicitly frozen, begs the question
what happens to recursive imports? For example, if we freeze
`Module3` which happens to import `Module2`, should this fail
if `Module2` was not frozen, or should it propagate to `Module2`
since we are already freezing modules?

If freezing modules does not propagate, then freeze needs to take
multiple arguments to permit resolving cyclic dependencies. For
example, if `Module2` also imports `Module3`, the only way to
freeze both "atomically" is through `freeze(Module2,
Module1.Module3)`.


## Do not implicitly freeze types

Analogous to the discussion about module objects above, we could
stop (and fail) if freezing encounters a non-frozen type. To
freeze a type, the type should be explicitly frozen. In this case,
`freeze(y)` would fail to freeze `O2` since its type was not
frozen. Since freezing a type requires its supertypes to be
frozen, it is probably convenient to let explicit type freezing
propagate to at least super types. While there are no cyclic
inheritance dependencies, there can be other cycles if two types
mention each other. This again requires `freeze(T1, T2)` unless
freezing propagates.
