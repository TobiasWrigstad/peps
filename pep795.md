...
## What we propose (Alternative 2.1)

This is a WIP based on Alternative 2. It is very close
but slightly simpler to explain.
...
`immutable` library provides a `@not_freezable` decorator.
...
- It either explicitly opts-in support for freezing or does
  not declare any class state
...
unless the class explicitly opts-in freezing support. This
...
Although `3.14` was already an immutable object, declaring the
class as freezable prevents reassigning `constant_state`.
